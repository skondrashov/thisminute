	...
	for (const auto &pair : wordCountPerCell)
	{
		...
		double globalWordRate = 0;
		{
			...
			for (int i = 0; i < MAP_WIDTH; i++)
			{
				for (int j = 0; j < MAP_HEIGHT; j++)
				{
					...
					{
						localWordRates[i][j] = (double)grid[i][j] / tweetCountPerCell[i][j];
		...
		for (int j = 0; j < MAP_HEIGHT; j++)
		{
			for (int i = 0; i < MAP_WIDTH; i++)
			{
				...
				values[j*MAP_WIDTH+i+1]  = to_string(wordCountPerCell[word][i][j]);
			}
		}
...
unordered_map<string, vector<vector<double>>>  getHistoricWordRates()
{
	unordered_map<string, vector<vector<double>>> historicWordRates;
